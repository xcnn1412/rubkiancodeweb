...
export default async function ServiceDetailPage({ params }: RouteProps) {
  ...
  return (
    <main className="min-h-screen bg-[#F4EDE0] text-[#0A2540]">
      ...
      {/* Key Feature Deep Dive — เน้น USP เด่น 1 ตัว */}
      {service.keyFeature && (
        <section className="border-t-[3px] border-[#0A2540] bg-white py-14 sm:py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-10 lg:grid-cols-[5fr_6fr] lg:items-center lg:gap-14">
              {/* Left — Image preview with chrome */}
              <div className="relative">
                ...
                >
                  <Image
                    src={service.keyFeature.image.src}
                    alt={service.keyFeature.image.alt}
                    fill
                    sizes="(max-width: 1024px) 100vw, 600px"
                ...
                <p className="font-pixel mt-4 text-center text-[10px] uppercase tracking-widest text-[#0A2540]/55">
                  ★ {service.keyFeature.image.caption}
                </p>
              </div>

              {/* Right — Title + description + benefit cards */}
              <div>
                <span
                  className="font-pixel inline-block bg-[#0A2540] px-3 py-2 text-[10px] uppercase tracking-widest text-[#F1C40F]"
                  style={{ boxShadow: "4px 4px 0 " + service.accent }}
                >
                  {service.keyFeature.eyebrow}
                </span>
                <h2 className="mt-5 text-3xl font-black uppercase leading-tight text-[#0A2540] sm:text-4xl lg:text-5xl">
                  {service.keyFeature.title}
                  <br />
                  <span style={{ color: service.accent }}>
                    {service.keyFeature.highlightedTitle}
                  </span>
                </h2>
                <p className="mt-5 text-base leading-relaxed text-[#0A2540]/80 sm:text-lg">
                  {service.keyFeature.description}
                </p>

                {/* Benefit cards — 4 ข้อ */}
                <ul className="mt-8 grid gap-4 sm:grid-cols-2">
                  {service.keyFeature.benefits.map((b, i) => (
                    <li
                      key={b.title}
                      className="bg-[#F4EDE0] p-4 transition-transform hover:-translate-x-0.5 hover:-translate-y-0.5"
                      style={{
                        border: "2px solid #0A2540",
                ...
                {/* Bottom hint — pixel-art arrow + tagline */}
                <div
                  className="mt-6 flex items-center gap-3 bg-[#0A2540] p-4"
                  style={{ boxShadow: "4px 4px 0 " + service.accent }}
                >
                  <span
                  ...
                  <p className="text-sm font-bold text-white sm:text-base">
                    เลิกเดา — เริ่มตัดสินใจจาก{" "}
                    <span style={{ color: service.accent }}>data จริง</span>{" "}
                    ทุกบาทที่ลงโฆษณา
                  </p>
                </div>
              </div>
